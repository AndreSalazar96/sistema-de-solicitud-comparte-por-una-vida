...
router.get('/user', isLoggedIn, async (req, res) => {
    const list_users = await pool.query('SELECT users.id_usuario, tipo_usuario.descripcion_tipo_usuario, users.fullname, users.username, users.telefono, users.correo, users.direccion, users.nombre_razon, users.id_tipo_usuario, users.create_at, users.avatar_image FROM users INNER JOIN tipo_usuario ON tipo_usuario.id_tipo_usuario = users.id_tipo_usuario');
  ...
  router.post('/updateuser/:id_usuario', isLoggedIn, async (req,res) => {
    const {id_usuario} = req.params;
    const {fullname, username, nombre_razon, id_tipo_usuario, telefono, correo, direccion, avatar_image} = req.body;
    const editUser = {
      fullname,
      username,
      nombre_razon,
